...
export class ActivateLoginGuard implements CanActivate {
  ...
  canActivate(next: ActivatedRouteSnapshot,state: RouterStateSnapshot):Observable<boolean | UrlTree> | Promise<boolean | UrlTree> | boolean | UrlTree {
        ...
        //si está logueado no se puede mostarr la pantalla de login y nos vamos a la pantalla de administradores
        if(this.generalService.logueado){
          ...
          this.datosService.getHeaderCall(this.parametrosService.baseUrlApi+"login",apitoken).subscribe(data => {
            ...
            //Si se está logueado se hacen las operaciones necesarias y se reddirige a administradores
            ...
            //Si no se está logueado se permite estar en el login al login
